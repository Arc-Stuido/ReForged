                            ...
                            ctx.getSource().sendSuccess(() -> Component.translatable("commands.forge.mods.list",
                                    ModList.get().applyForEachModFile(modFile ->
                                            ...
                                            String.format(Locale.ROOT, "%s %s : %s (%s) - %d",
                                                    (modFile.getProvider() != null ? modFile.getProvider().name() : "unknown_provider").replace(' ', '_'),
                                                    modFile.getFileName(),
                                                    !modFile.getModInfos().isEmpty() ? modFile.getModInfos().get(0).getModId() : "unknown_mod",
                                                    !modFile.getModInfos().isEmpty() ? modFile.getModInfos().get(0).getVersion() : "unknown_version",
